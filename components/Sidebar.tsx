              ...
              {groupItems.map(item => {
                ...
                return (
                  ...
                  >
                    {/* Priority dot */}
                    <span
                      className={`mt-1.5 w-2 h-2 rounded-full flex-shrink-0 ${PRIORITY_DOT[item.priority ?? ''] ?? 'bg-gray-300'}`}
                    />
                    <div className="flex-1 min-w-0">
                      ...
                      {item.legCity && (
                        <div className="text-xs text-gray-400 truncate mt-0.5">{item.legCity}</div>
                      )}
                      {!hasCords && (
